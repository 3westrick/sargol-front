...
const Coupons = () => {
  ...
  const coupons_query = useQuery({
      queryKey: ['admin-coupons', {query, field, order, limit: paginationModel.pageSize, offset: paginationModel.page * paginationModel.pageSize}],
      queryFn: () => getCoupons(query, field, order, paginationModel.pageSize, paginationModel.page * paginationModel.pageSize),
  })
  const {data, isLoading, isFetched} =  coupons_query
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'type', headerName: 'Coupon type', width: 250 ,
      valueGetter:({id, row, value}) => {
        if (value == 'percentage') return "Percentage discount"
        if (value == 'fixed_basket') return "Fixed basket discount"
        if (value == 'fixed_product') return "Fixed product discount"
        return value
      },
    },
  ...
  return (
    <>
        <Typography><Link href={'coupons/create'}>Create Coupons</Link></Typography>
